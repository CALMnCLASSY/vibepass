...
export default function DuwaraClient({ initialEvents }: { initialEvents: EventType[] }) {
    ...
    const fallbackImage = "/images/Duwara.jpeg";
    const description = "Duwara brings together the best of Afrohouse and Amapiano for a powerful celebration of African sound. In partnership with Masshouse, this experience will feature a curated lineup of top local DJs and featured artist Xduppy from South Africa, delivering deep grooves, soulful melodies, and dancefloor-shaking log drums. Duwara is a sonic journey for house heads and Amapiano lovers.";
    
    const displayEvents = hasData ? initialEvents.sort((a,b) => a.price - b.price) : [
        { id: 'd1', name: 'DUWARA - ADVANCE', price: 2000, date: '2026-04-03', location: 'Masshouse', imageUrl: fallbackImage, description, organizer: 'MADFUN + MASSHOUSE', longDescription: '' },
        { id: 'd2', name: 'DUWARA - GATE', price: 3000, date: '2026-04-03', location: 'Masshouse', imageUrl: fallbackImage, description, organizer: 'MADFUN + MASSHOUSE', longDescription: '' },
        { id: 'd3', name: 'DUWARA - PAIR TICKETS', price: 3500, date: '2026-04-03', location: 'Masshouse', imageUrl: fallbackImage, description, organizer: 'MADFUN + MASSHOUSE', longDescription: '' },
        { id: 'd4', name: 'DUWARA - VIP', price: 5000, date: '2026-04-03', location: 'Masshouse', imageUrl: fallbackImage, description, organizer: 'MADFUN + MASSHOUSE', longDescription: '' }
    ];
    ...
    const handleCheckout = async () => {
        ...
        const ticketSummary = displayEvents
            ...
            .map(e => `${quantities[e.id]}x ${e.name.replace('DUWARA - ', '')}`)
            .join(', ');
        ...
        fetch('/api/notify-discord', {
            ...
            body: JSON.stringify({
                type: 'payment',
                embed: {
                    title: "⏳ Duwara Payment Initialized",
        ...
        paystack.newTransaction({
            ...
            currency: 'KES',
            ref: `vibe-pass-duwara-${Date.now()}-${Math.floor(Math.random() * 1000000)}`,
            metadata: {
                custom_fields: [
                ...
                try {
                    ...
                    fetch('/api/notify-discord', {
                        ...
                        body: JSON.stringify({
                            type: 'payment',
                            embed: {
                                title: "✅ Duwara Payment Successful",
                    ...
                    const res = await fetch('https://classybooks.onrender.com/api/verify-payment', {
                        ...
                        body: JSON.stringify({
                            ...
                            eventId: 'DUWARA-MULTIPLE',
                            eventName: `Duwara (${ticketSummary})`,
                            customerName: name,
                            customerEmail: email,
                            customerPhone: phone,
                            amount: totalAmount,
            ...
            onCancel: () => {
                ...
                fetch('/api/notify-discord', {
                    ...
                    body: JSON.stringify({
                        type: 'payment',
                        embed: {
                            title: "❌ Duwara Payment Cancelled",
                            color: 0xef4444, // Red
                            fields: [
                                { name: "👤 Customer", value: name, inline: true },
                                { name: "📧 Email", value: email, inline: true },
        ...
        return (
            <div className="min-h-screen bg-gray-50 flex items-center justify-center p-4">
                <div className="bg-white p-8 rounded-2xl shadow-xl max-w-md text-center">
                    ...
                    <button onClick={() => setSuccessMessage(null)} className="bg-amber-600 text-white px-6 py-2 rounded-lg font-medium">Buy More Tickets</button>
                </div>
            </div>
        );
    }
                    ...
                    <div className="block md:grid md:grid-cols-5 gap-8">
                        ...
                        <div className="md:hidden mb-6 flex justify-center">
                            <div className="w-full aspect-square rounded-xl overflow-hidden shadow-2xl relative border border-white/10">
                                <img className="w-full h-full object-cover" src={displayEvents[0]?.imageUrl || fallbackImage} alt="Duwara" />
                            </div>
                        </div>

                        {/* Title & Info */}
                        <div className="md:col-span-3 mt-3 md:mt-0">
                            <div className="text-3xl md:text-5xl font-black tracking-tight" style={{ fontFamily: "'Montserrat', sans-serif" }}>Duwara</div>
                            <div className="text-sm md:text-lg mb-2 font-medium mt-4 flex gap-6 md:gap-8 border-b border-white/10 pb-4 md:border-none md:pb-0">
                                <span className="text-amber-400 font-bold whitespace-nowrap">03 Apr</span>
                                <span className="opacity-70 whitespace-normal">Masshouse</span>
                            </div>
                ...
                <div className="md:grid md:grid-cols-3 bg-white border border-gray-200 md:rounded-2xl overflow-hidden shadow-xl">
                    ...
                    <div className="md:col-span-2 min-h-fit md:border-r border-gray-200">
                        {displayEvents.map((tier) => {
                            const nameRender = tier.name.replace('DUWARA - ', '');
                            ...
                            return (
                                <div key={tier.id} className="flex flex-col sm:flex-row sm:items-center px-4 md:px-6 py-5 border-b border-gray-100 hover:bg-gray-50 transition-colors">
                                    <div className="flex-grow mb-3 sm:mb-0">
                                        ...
                                        <div className="text-sm font-medium text-amber-600 mt-1">KES {tier.price.toLocaleString()}</div>
                                    </div>
                                    <div className="flex items-center self-start sm:self-center">
                                        <button 
                                            className="w-10 h-10 bg-amber-100 text-amber-600 rounded-md flex items-center justify-center font-bold text-xl hover:bg-amber-200 transition-colors disabled:opacity-50"
                                            onClick={() => updateQuantity(tier.id, -1)}
                                            disabled={curQty <= 0}
                                        >-</button>
                                        <div className="w-12 text-center font-bold text-lg text-gray-800">{curQty}</div>
                                        <button 
                                            className="w-10 h-10 bg-amber-600 text-white rounded-md flex items-center justify-center font-bold text-xl hover:bg-amber-700 transition-colors disabled:opacity-50"
                                            onClick={() => updateQuantity(tier.id, 1)}
                                            disabled={curQty >= 10}
                                        >+</button>
                                    </div>
                    ...
                    <div className="relative bg-gray-50 flex flex-col">
                        <div className="p-4 md:p-6 bg-white border-b border-gray-200 font-bold text-gray-800 flex justify-between items-center sticky top-0 z-10 hidden md:flex">
                            <span>Summary</span>
                            <span className="text-amber-600">KES {totalAmount.toLocaleString()}</span>
                        </div>
                            ...
                            <div className="mb-6">
                                ...
                                <div className="space-y-4">
                                    <div>
                                        ...
                                        <input className="w-full border border-gray-300 rounded-md px-4 py-2 text-sm bg-white focus:ring-2 focus:ring-amber-500 focus:border-transparent outline-none transition-all" type="text" placeholder="John Doe" value={name} onChange={e => setName(e.target.value)} />
                                    </div>
                                    <div>
                                        <label className="block text-xs font-semibold text-gray-600 mb-1">Email Address</label>
                                        <input className="w-full border border-gray-300 rounded-md px-4 py-2 text-sm bg-white focus:ring-2 focus:ring-amber-500 focus:border-transparent outline-none transition-all" type="email" placeholder="john@example.com" value={email} onChange={e => setEmail(e.target.value)} />
                                    </div>
                                    <div>
                                        <label className="block text-xs font-semibold text-gray-600 mb-1">Phone Number</label>
                                        <input className="w-full border border-gray-300 rounded-md px-4 py-2 text-sm bg-white focus:ring-2 focus:ring-amber-500 focus:border-transparent outline-none transition-all" type="tel" placeholder="+254700000000" value={phone} onChange={e => setPhone(e.target.value)} />
                                    </div>
                                </div>
                            </div>
                        </div>
                        ...
                        <div className="p-4 md:p-6 bg-white border-t border-gray-200 mt-auto sticky bottom-0 z-20 shadow-[0_-10px_15px_-3px_rgba(0,0,0,0.05)] md:shadow-none">
                            <button 
                                ...
                                className={`w-full py-4 rounded-xl font-bold tracking-wide transition-all ${
                                    ...
                                    ? 'bg-amber-400 text-black hover:bg-amber-500 shadow-lg hover:shadow-xl transform hover:-translate-y-0.5' 
